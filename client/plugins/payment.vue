...
import { mapGetters } from 'vuex';
  export default {
    ...
    mounted(){
      this.stripe = Stripe("pk_test_51HyacDJi1PTzZZm5a50YAgBwOdPKDsJu3yZosvpfgAcu2wivromrWHPwgvT2OzeveW3cfx0xuTTTPqQRNTpGHzuT00hXsMPUes")
   ...
   methods : {
    async onPurchase(){
      try {
        ...
        if(response.success){
            this.store.$state.commit("clearCart")
          this.$router.push("/")
        }
      } catch (error) {
        console.log(error)
